---
title: "CosMx Whole Transcriptome: Human Colon Mini Dataset"
tags:
  size: mini
  platform: CosMx
  panel: "5,000+"
...
pkgdown:
  as_is: true
vignette: >
  %\VignetteIndexEntry{CosMx Whole Transcriptome: Human Colon Mini Dataset}
...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, message = FALSE,
                      fig.align = "center", dpi = 130, fig.path = "figures/")
set.seed(42)
```

# The dataset

NanoString **CosMx Whole Transcriptome (WTx)**, human colon, slide S0. This is a
**single field of view** (FOV 71) cut out of the ~400-FOV export the companion
GiottoDisk tutorial works through. The donor is a 56-year-old woman with a G1,
stage **IVA T3N0M1a** primary sigmoid adenocarcinoma, and the field was chosen
where tumor, desmoplastic stroma and residual mucosa meet.
...
The parent export is Bruker Spatial Biology's
...
The subset is registered with `GiottoData` as **`cosmx_mini_colon_wtx`**, so
`getSpatialDataset()` fetches and unpacks it in one call.
...
`getSpatialDataset()` downloads the bundle, checks it against the checksum in
`GiottoData`'s manifest and unpacks it, then returns the directory it used.
Either way the download is skipped on a re-run.
...
into the cell metadata, which is what the thresholds below act on. Running it on
`negprobes` too gives the background against which the RNA counts should be
read: the 50 negative probes carry no target, so their per-cell totals are the
noise floor for this panel.
...
<details>
<summary><strong>Pre-filter distributions and an overlap cross-check</strong></summary>

Transcripts carry the vendor's own cell assignment, so re-deriving the counts by
overlapping them with the segmentation is an independent check on both that
assignment and the polygon repair.

```{r overlap-qc, eval = FALSE}
...
```{r overlap-qc-fig, fig.cap="Vendor counts against overlap-derived counts, one point per cell; the dashed line is equality.", fig.width=5.2, fig.height=4.6, eval = FALSE}
...
```{r overlap-qc-fig-fig, echo = FALSE, out.width = "100%"}
knitr::include_graphics("images/cosmx_mini_colon_wtx/01_overlap-qc-fig.png")
```

Derived totals run slightly high because ~7% of detections are unassigned by
the vendor yet still fall inside a polygon. Scatter off the diagonal reflects
the polygon repair, since a few microns of residual placement error moves
boundary transcripts into a neighboring cell. A much lower correlation would
point at the repair rather than the assignment.

`filterDistributions()` reads the per-cell statistics straight off the object,
so the two panels below are the distributions before any filtering.

```{r qc-figs, fig.cap="Counts and genes per cell before filtering, with the gene threshold drawn.", fig.width=9, fig.height=3.2, eval = FALSE}
...
knitr::include_graphics("images/cosmx_mini_colon_wtx/02_qc-figs.png")
```
...
knitr::include_graphics("images/cosmx_mini_colon_wtx/03_scree.png")
```
...
```{r cluster-figs, fig.cap="Every cell in UMAP space by Leiden cluster, and the same embedding colored by genes detected.", fig.width=11, fig.height=4.6, eval = FALSE}
...
knitr::include_graphics("images/cosmx_mini_colon_wtx/04_cluster-figs.png")
```

# Markers

`findScranMarkers_one_vs_all()` compares each cluster against the pooled
remainder with a Welch test and returns a `ranking` column, which orders the
genes within each cluster.

> **Disclaimer: this is a 1,752-cell fixture, not a study.** One field of view cannot
> support biological conclusions: clusters are small, rare populations are
> absent or represented by a handful of cells, and the marker statistics are
> correspondingly unstable. Treat everything downstream as a demonstration
> of the pipeline, and do not read the cell-type calls as findings.
...
One curated gene set per broad cell type, used to read the clusters below and to
pick the genes drawn in the dot plot.
...
One entry per Leiden cluster. If this is re-run the cell typing may change,
because Leiden numbers carry no meaning across runs.
...
```{r dotplot, fig.cap="Curated panel genes by cell type: dot size is percent of cells detecting the gene, color is mean normalized expression.", fig.width=5.5, fig.height=8, eval = FALSE}
...
knitr::include_graphics("images/cosmx_mini_colon_wtx/05_dotplot.png")
```

```{r meta-heatmap, fig.cap="Rescaled z-scores of the top de novo markers across cell types.", fig.width=8, fig.height=7, eval = FALSE}
...
```{r meta-heatmap-fig, echo = FALSE, out.width = "100%"}
knitr::include_graphics("images/cosmx_mini_colon_wtx/06_meta-heatmap.png")
```

```{r violin, fig.cap="One canonical gene per cell type, by cell type.", fig.width=9, fig.height=6, eval = FALSE}
violinPlot(mini_cosmx, feats = MARKERS, cluster_column = "cell_type_broad",
           expression_values = "normalized", strip_text = 7,
           axis_text_x_size = 7)
```

```{r violin-fig, echo = FALSE, out.width = "100%"}
knitr::include_graphics("images/cosmx_mini_colon_wtx/07_violin.png")
```

```{r dimfeat, fig.cap="Marker expression on the UMAP embedding.", fig.width=9, fig.height=7, eval = FALSE}
dimFeatPlot2D(mini_cosmx, expression_values = "normalized",
              feats = MARKERS, dim_reduction_to_use = "umap",
              point_size = 1, cow_n_col = 2)
```

```{r dimfeat-fig, echo = FALSE, out.width = "100%"}
knitr::include_graphics("images/cosmx_mini_colon_wtx/08_dimfeat.png")
```
...
knitr::include_graphics("images/cosmx_mini_colon_wtx/09_umap-celltype.png")
```

```{r spatial-celltype, fig.cap="The field by cell type, drawn as segmentation polygons.", fig.width=8, fig.height=7, dpi=200, eval = FALSE}
...
knitr::include_graphics("images/cosmx_mini_colon_wtx/10_spatial-celltype.png")
```

The clustering never saw spatial information, so the tissue architecture in
this panel is an independent check on it.

```{r spatial-facets, fig.cap="One panel per cell type, every other cell in gray behind it.", fig.width=11, fig.height=7, eval = FALSE}
...
knitr::include_graphics("images/cosmx_mini_colon_wtx/11_spatial-facets.png")
```

```{r spat-feat, fig.cap="IGKC (left) and COL1A1 (right) as polygon fill, normalized expression.", fig.width=11, fig.height=5, eval = FALSE}
...
```{r spat-feat-fig, echo = FALSE, out.width = "100%"}
knitr::include_graphics("images/cosmx_mini_colon_wtx/12_spat-feat.png")
```

# Transcripts in place

```{r transcripts, fig.cap="Epithelial, endothelial and plasma-cell transcripts over the segmentation.", fig.width=8.5, fig.height=7.5, eval = FALSE}
spatInSituPlotPoints(mini_cosmx,
...
knitr::include_graphics("images/cosmx_mini_colon_wtx/13_transcripts.png")
```

# Spatial expression patterns

A Delaunay network over the cell centroids, then `binSpect()`: each gene's
expression is binarized and tested for whether the high cells are neighbors more
often than chance.
...
```{r binspect-figs, fig.cap="The six top-scoring spatially patterned genes, normalized expression.", fig.width=9, fig.height=6, eval = FALSE}
...
```{r binspect-figs-fig, echo = FALSE, out.width = "100%"}
knitr::include_graphics("images/cosmx_mini_colon_wtx/14_binspect-figs.png")
```

```{r spatcor, eval = FALSE}
...
```{r spatcor-heatmap, fig.cap="Genes with correlated spatial expression, grouped into modules.", fig.width=8, fig.height=7, eval = FALSE}
...
```{r spatcor-heatmap-fig, echo = FALSE, out.width = "100%"}
knitr::include_graphics("images/cosmx_mini_colon_wtx/15_spatcor-heatmap.png")
```

```{r spatcor-modules, eval = FALSE}
...
knitr::include_graphics("images/cosmx_mini_colon_wtx/16_cp-barplot.png")
```

```{r cp-heatmap, fig.cap="Proximity enrichment for every cell type pair.", fig.width=7.5, fig.height=6.5, eval = FALSE}
cellProximityHeatmap(mini_cosmx, CPscore = proximity, order_cell_types = TRUE,
                     scale = TRUE)
```

```{r cp-heatmap-fig, echo = FALSE, out.width = "100%"}
knitr::include_graphics("images/cosmx_mini_colon_wtx/17_cp-heatmap.png")
```

The heatmap shows every pair at once, so the self-association above appears as
a bright diagonal; off-diagonal warmth marks types that interleave, and cool
off-diagonal cells mark compartments that exclude one another.

```{r cp-network, fig.cap="The same scores as a graph; edge weight is enrichment.", fig.width=7.5, fig.height=6.5, eval = FALSE}
...
knitr::include_graphics("images/cosmx_mini_colon_wtx/18_cp-network.png")
```

Edge thickness scales with the size of the score and edge color gives its sign:
red where a pair is found together more often than chance, light green where it
is found together less. Dropping the diagonal makes chains of mutually adjacent
cell types easier to see than isolated pairs.

# Saving the object

`saveGiotto()` writes a whole Giotto project (the object, its expression and
its spatial subobjects) into one self-contained directory, which `loadGiotto()`
