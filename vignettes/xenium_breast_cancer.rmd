...
For a more recent example, see [Xenium FFPE Human Lung Cancer with Multimodal Cell Segmentation](https://giotto-suite.github.io/giotto_workshop_2024/xenium-1.html)
...
<details><summary>Alternative data to load</summary>
...
<details>
<summary>Loading from non-standard directories or other provided file formats</summary>
...
<details>
<summary>qv_threshold setting</summary>
...
<details>
<summary>feat_types and split_keywords</summary>
...
<details>
<summary>**Gene expression** (313) - These are the `'rna'` gene detections.</summary>
```{r, eval=FALSE}
...
<details>
<summary>**Blank Codeword** (159) - (QC) Codewords that should not be used in the current panel. (named **Unassigned Codeword** in later Xenium pipelines)</summary>
```
...
<details>
<summary>**Negative Control Codeword** (41) - (QC) Codewords that do not map to genes, but are in the codebook. Used to determine specificity of decoding algorithm</summary>
```
...
<details>
<summary>**Negative Control Probe** (28) - (QC) Probes in panel but target non-biological sequences. Used to determine specificity of assay.</summary>
```
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("images/xenium_breast_cancer/dimfeats.png")
```

These markers are not very specific to the UMAP clustering. Several genes were
also not available in the panel.

# Single Cell Assisted Cell Typing

Xenium datasets have very sensitive target detection, but segmentation plays a
large role in how clean the expression information is. This dataset did not come with
bound staining segmentation and the `'cell'` segmentation is instead a simple
expansion-based approach performed on top of the `'nucleus'` annotations. 
As a result, many transcripts may be assigned to
the wrong cell.

Single cell datasets lack spatial context, however they tend to have clearer
separation between clusters, making it easier to see which clusters correspond
to which cell types.
This dataset comes with paired single cell datasets at 
[GSE243280](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE243280). 
We can use the single cell data to cell type, then transfer those labels to the
Xenium data.
...
<details><summary>single cell data processing</summary>
...
knitr::include_graphics("images/janesick_sc/1_mito.png")
```
...
```{r, echo=FALSE, out.width="40%"}
knitr::include_graphics("images/janesick_sc/2_mito.png")
```
...
```{r, echo=FALSE, out.width="40%"}
knitr::include_graphics("images/janesick_sc/3_doublet.png")
```
...
knitr::include_graphics("images/janesick_sc/4_filter_combinations.png")
```
...
```{r, echo=FALSE, out.width="80%"}
knitr::include_graphics("images/janesick_sc/5_scree.png")
```
...
```{r, echo=FALSE, out.width="80%"}
knitr::include_graphics("images/janesick_sc/6_umap.png")
```
...
```{r, echo=FALSE, out.width="80%"}
knitr::include_graphics("images/janesick_sc/7_overclust.png")
```
...
```{r, echo=FALSE, out.width="80%"}
knitr::include_graphics("images/janesick_sc/8_mclus.png")
```
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("images/janesick_sc/9_dimfeats.png")
```
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("images/janesick_sc/10_scran.png")
```
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("images/janesick_sc/11_gini.png")
```
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("images/janesick_sc/12_celltypes.png")
```
...
Next, a joined Xenium and single cell `giotto` object needs to be created to transfer
...
## Process Joined Object (No Integration)
...
```{r, echo=FALSE, out.width="80%"}
knitr::include_graphics("images/xenium_breast_cancer/join_pca.png")
```
...
```{r, echo=FALSE, out.width="80%"}
knitr::include_graphics("images/xenium_breast_cancer/join_scree.png")
```
...
```{r, echo=FALSE, out.width="80%"}
knitr::include_graphics("images/xenium_breast_cancer/join_umap.png")
```
...
Run Harmony integration on the PCA space with the `"list_ID"` (dataset source) as the variable to remove to
produce an aligned PCA space that better represents the biology of the datasets.
...
```{r, echo=FALSE, out.width="80%"}
knitr::include_graphics("images/xenium_breast_cancer/join_celltypes.png")
```

The single-cell-based celltype annotations look like they map to the clustering
of the integrated dataset. We can now transfer these annotations using `labelTransfer()`
from just the  single cells (`source_cell_ids`) to the rest of the cells (xenium data cells)
...
```{r, echo=FALSE, out.width="80%"}
knitr::include_graphics("images/xenium_breast_cancer/join_transfer.png")
```
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("images/xenium_breast_cancer/xen_transfer.png")
```
