...
The K-Nearest Neighbors (kNN) algorithm operates on the principle of likelihood of similarity. It posits that similar data points tend to cluster near each other in space. You can find more information about this method [here.](https://link.springer.com/article/10.1186/s40537-024-00973-y) By default, createNearestNetwork() calculates the 30 K-Nearest Neighbors to each spot/cell, but you can modify this number using the "k" parameter.
...
This algorithm is more complicated than the Louvain algorithm and have an accurate and fast result for the computation time. The Leiden algorithm consists of three phases. The first phase is the modularity optimization process, the second phase is the refinement of partition, and the third phase is the community aggregation process. This algorithm performs well on small, medium and large-scale networks.

You can find more information about Louvain and Leiden clustering [here](https://iopscience.iop.org/article/10.1088/1742-6596/2129/1/012028/pdf)
...
PAGE matrix should be a binary matrix with each row represent a gene marker and each column represent a cell type. There are several ways to create PAGE matrix
...
PAGE_matrix <- makeSignMatrixPAGE(sign_names = c("Granule_neurons",
                                                 "Oligo_dendrocytes",
                                                 "di_mesenchephalon"),
...
- 3. make PAGE matrix from single cell dataset
...
Use the rank binarization. Rank the genes on the spatial dataset depending on whether they exhibit a spatial pattern location or not. This step may take a few minutes to run.
