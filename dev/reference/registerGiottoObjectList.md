...
- method:
  ...
  Method used to align gobjects. Current options are either using FIJI
  register_virtual_stack_slices output or rvision
...
- image_reg_name:
  ...
  Arbitrary image slot name for registered images to occupy. Defaults to
  replacement of "image" slot (optional)
...
- spatloc_unreg:
  ...
  Unregistered spatial locations to align. Defaults to 'raw' slot
  (optional)

- spatloc_reg_name:

  Arbitrary name for registered spatial locations. Defaults to
...
- allow_rvision_autoscale:

  Whether or not to allow rvision to automatically scale the images when
