...
- reduce_resolution:
  ...
  numeric; the original Dapi image from Vizgen works better in the
  Mesmer algorithm if its resolution is reduced 4 times.

- overlapping_pixels:

  numeric; the number of pixels to overlap when calculating the rolling
  window
