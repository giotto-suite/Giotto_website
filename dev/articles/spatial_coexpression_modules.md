...
Once you have found Finding genes or features that follow spatial
patterns in their expression, you can group them to find clusters with
similar expression patterns that create co-expression modules.
...
## 2 Select the top spatial genes
...
## 3 Calculate pairwise distances between genes.
...
## 4 Cluster the top spatial genes into 5 clusters
...
## 5 Plot the correlation of the top spatial genes with their assigned cluster.
...
![](images/spatial_coexpression_modules/1-heatmSpatialCorFeats.png)

### 5.1 Rank spatial correlated clusters and show genes for selected clusters
...
![](images/spatial_coexpression_modules/2-rankSpatialCorGroups.png)

### 5.2 Create the metagene enrichment score per co-expression cluster
...
Plot the spatial distribution of the metagene enrichment scores of each
spatial co-expression cluster.
...
![](images/spatial_coexpression_modules/3-spatCellPlot2D.png)

## 6 Get the top genes per spatial co-expression module

Run this code for downstream analysis, such as calculation of spatially
informed clusters or calculation of spatial domains with HMRF.

In a real-size dataset, usually you want to extract the top ~30 genes
per co-expression module, but this number is variable depending on the
number of features available in your dataset.
