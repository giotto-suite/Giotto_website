...
Cross sections in {Giotto} are used to generate an in-silico cross
section of a 3D dataset along an arbitrary 2D plane.

A plane may be defined in one of four ways:

`"equation"` - Provide a vector of 4 numerics to `equation`. These will
be taken as the A, B, C, D coefficients in the plane-defining function
Ax + By + Cz = D

This method is shown below.

`"3 points"` - Provide 3D coordinates to `point1`, `point2`, `point3`
params. For example:
...
`"point and norm vector"` - Provide a 3D point anchor, then a 3D vector
normal to the plane to set the plane rotation. For example:

        method = "point and norm vector",
        point1 = c(100, 200, 300),
        normVector = c(0, 1, 0)  # Normal points in y direction (horizontal plane)

`"point and two plane vectors"` - Provide a 3D point anchor and two
non-parallel 3D vectors that define the plane rotation. For example:
...
For the example, we use a mini premade `giotto` object. This was mini
dataset is a randomly sampled subset of a STARmap mouse brain dataset.
...
Next we define the cross section to create using the `"equation"`
method.
...
This cross section can then be used in cross section-specific plotting
functions.
...
![](images/cross_section/1-crossSectionPlot.png)
...
![](images/cross_section/2-crossSectionPlot3Dpng.png)
...
![](images/cross_section/3-crossSectionGenePlot.png)
...
![](images/cross_section/4-crossSectionGenePlot3D.png)
