...
## 1 Expression Value Processing Pipeline

{Giotto} `v4.2.1` and {GiottoClass} `v0.4.7` add a composable framework
for data normalization and transformation with 2 main components:

- [`processExpression()`](https://giottosuite.com/dev/reference/processExpression.md) -
  `giotto` object-level function for **working with the expression
  values**. Passes to
  [`processData()`](https://giottosuite.com/dev/reference/processData.md)
  after handling any `giotto` object-level data extraction/processing.
  - [`processData()`](https://giottosuite.com/dev/reference/processData.md) -
    lower-level generic that handles specific operations at the data
    representation level.
- Factory functions for method-specific **parameter classes** that both
  define the operation to perform and also contain all relevant
  parameters in an editable format. For the full set of available
  methods, see
...
Detailed information about the implementation details for each method is
available on the individual method help pages. the
[`?process_param`](https://giottosuite.com/dev/reference/process_param.md)
documentation page is a central directory that links to specific
methods. These include the mathematical formula description for
statistical methods and any parameters that can be used. These pages are
useful resources for understanding the specific transformations being
applied to your data.

*This workflow supercedes the original
...
Get param settings in a parameter class using the `$` operator.
Available params will tab autocomplete. Param definitions are also
documented in the help pages of specific parameter classes.

See also:

- [`?process_param`](https://giottosuite.com/dev/reference/process_param.md) -
  central directory of provided processing parameters
...
### 1.2 Chained Operations

**These operations can additionally be chained together when provided as
list:**
...
### 2.1 Default Normalization and Scaling
...
With
[`processExpression()`](https://giottosuite.com/dev/reference/processExpression.md),
this can be rewritten as either of the two below:
...
### 2.2 Pearson Residual Normalization

A normalization method described in [Lause,
2021](https://doi.org/10.1186/s13059-021-02451-7), designed for
detection of highly variable features and dimension
reduction/clustering. This method includes a z-score-like
transformation, so scaling is not recommended after this. Using these
values for differential gene expression analysis is also not
recommended.
...
### 2.3 Arcsinh Transformation with Cofactor C

Arcsinh transformation is commonly used with spatial
proteomics/intensity type data. It is suited for data with a wide
dynamic range and zero/near-zero values and preserves detail across
multiple orders of magnitude, where a linear or log transform would
distort important features.

A **cofactor C** controls the transition between linear and logarithmic
behaviors. With CODEX or CyCIF, `c = 5` is commonly used. `c = 1` or `5`
is commonly used with IMC.

- Smaller C values (1-3): More aggressive transformation, useful for
  very wide dynamic ranges
- Larger C values (5-10): More gentle transformation, preserving more of
...
### 2.4 TF-IDF Normalization

TF-IDF (Term Frequency-Inverse Document Frequency) is commonly used in
ATAC-seq data, but is generally helpful in situations where the data is
very noisy. This normalization method pulls on concepts of natural
language processing to effectively focus in on features specific to
particular samples/cells.
...
## 3 Adding A Custom Method

Processing methods for this framework are defined for the
[`processData()`](https://giottosuite.com/dev/reference/processData.md)
generic from {GiottoClass} against **parameter class** and **data
class** combinations. **parameter classes** should inherit from the
`processParam` class defined in {GiottoClass}
