# Interactive region selection on large datasets

While the interactive region selection using the [Shiny
app](https://giotto-suite.github.io/Giotto_website/articles/interactive_selection.html)
is very user-friendly, it doesn’t offer the best performance when
handling big datasets.

We have created a function that facilitates the interaction with the
[vitessceR package](https://r-docs.vitessce.io) for interactive
visualization of processed large datasets.

## 1 Set up the Giotto enviroment
...
## 2 Install the vitessceR package
...
For this tutorial, we will use the [Nanostring CosMx Subcellular Lung
Cancer](https://giotto-suite.github.io/Giotto_website/articles/nanostring_cosmx_lung_cancer.html)
processed Giotto object.

## 4 Load the Giotto object

If you have the object already in your R environment, you can skip this
step. If you exported it to a folder using the
...
## 5 Export the Giotto object to a local Anndata-Zarr folder

By default, the function giottoToAnndataZarr() will look for the “cell”
spatial unit and “rna” feature type, but you can specify the spat_unit
and feat_type arguments, as well as the expression values to use.

In addition, you need to specify the path or name for creating a new
folder that will store the Anndata-Zarr information.
...
## 6 Create the vitessceR object

To create the vitessceR object, you need to provide the paths for the
metadata information that you want to load from your Anndata-Zarr
folder. We suggest to explore the subfolders obs (for cell metadata),
var (for feature metadata), and obsm (for spatial and dimension
reduction data).
...
## 7 Create the vitessceR schema

Here we will create the base of the schema using the previous object,
then we will add the components to generate interactive plots.
...
You can create multi-column layout and add more compartments using the
horizontal (hconcat) or vertical (vconcat) sections.
...
![](images/interactive_selection_cosmx/01_vitessceR.png)

## 10 Select the regions of interest

Use the lasso tool to select a region of interest.

![](images/interactive_selection_cosmx/02_lasso_tool.png)

After drawing the selection, the area will be highlighted and the
corresponding cells will be mapped and highlighted in the dimension
reduction plots.

![](images/interactive_selection_cosmx/03_selected_region.png)

You can see your selected regions in the Cell Sets panel, under the My
Selections group.

![](images/interactive_selection_cosmx/04_selections_window.png)

## 11 Export your selected regions

When you have finished selecting the regions of interest, click on the
dots menu and select the option Export hierarchy to csv.

![](images/interactive_selection_cosmx/05_export_to_csv.png)

## 12 Add the selected regions to the Giotto object

### 12.1 Read the selections file
...
![](images/interactive_selection_cosmx/06_selections_table.png)
...
### 12.2 Create a Giotto polygon object

We must transform the data.frame with coordinates into a Giotto polygon
object.
...
### 12.3 Add the cells within the polygons to the Giotto object
...
![](images/interactive_selection_cosmx/07_new_metadata.png)

## 13 Downstream analysis

### 13.1 Extract the cells within each selected region

By default, the function will retrieve the cells located within all the
selected regions. You can use the argument `polygons` to specify what
Selection you want to get, e.g “Selection 1”.
...
![](images/interactive_selection_cosmx/08_get_cells.png)

### 13.2 Compare the cell types between regions
...
![](images/interactive_selection_cosmx/09_compare_cells.png)

### 13.3 Compare the gene expression between regions
...
![](images/interactive_selection_cosmx/10_compare_genes.png)

Or you can calculate the top expressed genes per region and then plot
them.
...
![](images/interactive_selection_cosmx/11_scran_genes.png)

### 13.4 Plot again the selected regions

Use the ‘selections’ column in the metadata table to color the cells
with their corresponding Selection ID.
...
![](images/interactive_selection_cosmx/12_plot_selections.png)
