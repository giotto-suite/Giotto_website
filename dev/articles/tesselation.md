...
Subcellular data is organized in a grid format. We can aggregate the
...
for each transcript: - gene id - count or UMI - spatial pixel location
(x, y)
...
![Giotto polygon in a hexagon shape for overlapping visium HD expression
data](images/tesselation/1-hex.png)
...
## 5 Show the giotto points (transcripts) and polygons (hexagons) together
...
![Overlap of gene expression with the hex25 polygons. Each dot
represents a single gene. Jitter used to better vizualize individual
transcripts](images/tesselation/0-spatInSituPlotPoints.png)
